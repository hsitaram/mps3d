...
! config
!
  integer, parameter :: species_max = 5, species_electrons = 1, species_length = 4, reactions_max = 3, reactions_length = 14
...
!
  ...
  integer, parameter, private               :: vode_neq = species_max 
  ...
  double precision                          :: rrt(reactions_max), mrtm(species_max, reactions_max), ZDPlasKin_cfg(3)
  logical, private                          :: lZDPlasKin_init = .false., lprint, lstat_accum, ldensity_constant, &
                                               density_constant(species_max) 
!
! qtplaskin config
!
...
!
  ...
  /"E+HE=>HE*+E   ","E+HE=>HE**+E  ","E+HE=>HE^++E+E"/
contains
!-----------------------------------------------------------------------------------------------------------------------------------
!
! initialization
...
subroutine ZDPlasKin_set_config(ATOL,RTOL,SILENCE_MODE,STAT_ACCUM,QTPLASKIN_SAVE)
  ...
  implicit none
  logical, optional, intent(in) :: SILENCE_MODE, STAT_ACCUM, QTPLASKIN_SAVE
  double precision, optional, intent(in) :: ATOL, RTOL
  integer :: i
...
subroutine ZDPlasKin_set_conditions(GAS_TEMPERATURE,REDUCED_FREQUENCY,REDUCED_FIELD, &
                                    SOFT_RESET)
  implicit none
  double precision, optional, intent(in) :: GAS_TEMPERATURE, REDUCED_FREQUENCY, REDUCED_FIELD 
...
300 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file <" &
    ...
    close(ifile_unit)
311 format(171x,5(1x,i9))
312 format(A1,1x,A14,1x,5(1x,A9))
313 format(i1,1x,A14,1x,5(1x,1pd9.2))
  endif
  return
...
subroutine ZDPlasKin_write_qtplaskin(time,LFORCE_WRITE)
  ...
  integer, parameter :: idef_data = 2
  character(19), parameter :: qtplaskin_names(idef_data) = (/ "Reduced field [Td] ", "Gas temperature [K]" /)
...
100 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file " // &
    ...
    do i = 1, species_max
      write(ifile_unit,"(3(i3))",err=200) int(mrtm(i,:))
    enddo
    lerror = .false.
200 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file " // &
...
400 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file " // &
    ...
    write(ifile_unit,"(1x,A12,3(121x,i1.1))",err=500) "Time_s", ( i, i = 1, reactions_max )
    lerror = .false.
500 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file " // &
  ...
  if( ( time - densav(0,1) ) >= rtol .or. lfirst ) then
    ...
    if( rtol > qtplaskin_rtol .or. lfirst ) then
      ...
      write(ifile_unit,"(3(1pe13.4),$)") densav(0,2), cond(:)
      if( iuser_data > 0 ) then
      ...
      write(ifile_unit,"(4(1pe13.4))") densav(0,2), rrt_loc(:)
      close(ifile_unit)
      densav(:,1) = densav(:,2)
    endif
  endif
...
subroutine ZDPlasKin_reac_source_matrix(reac_rate_local,reac_source_local)
  ...
  reac_source_local(5,1) = - reac_rate_local(1) 
  reac_source_local(4,2) = + reac_rate_local(2) 
...
subroutine ZDPlasKin_jex(neq,t,y,ml,mu,pd,nrpd)
  ...
  density(:) = y(1:species_max)
  call ZDPlasKin_reac_rates(t)
  pd(3,1) = pd(3,1) + rrt(1) * density(5) 
  pd(3,5) = pd(3,5) + rrt(1) * density(1) 
  pd(5,1) = pd(5,1) - rrt(1) * density(5) 
  pd(5,5) = pd(5,5) - rrt(1) * density(1) 
  pd(4,1) = pd(4,1) + rrt(2) * density(5) 
  pd(4,5) = pd(4,5) + rrt(2) * density(1) 
  pd(5,1) = pd(5,1) - rrt(2) * density(5) 
  pd(5,5) = pd(5,5) - rrt(2) * density(1) 
  pd(1,1) = pd(1,1) + rrt(3) * density(5) 
  pd(1,5) = pd(1,5) + rrt(3) * density(1) 
  pd(2,1) = pd(2,1) + rrt(3) * density(5) 
  pd(2,5) = pd(2,5) + rrt(3) * density(1) 
  pd(5,1) = pd(5,1) - rrt(3) * density(5) 
  pd(5,5) = pd(5,5) - rrt(3) * density(1) 
...
subroutine ZDPlasKin_reac_rates(Time)
  use ZDPlasKin, only : ZDPlasKin_cfg, ZDPlasKin_get_density_total, &
                        lreaction_block, rrt
  implicit none
  double precision, intent(in) :: Time
  rrt(1) = EXP(-5.36094408D+01+(1.45540903D+00)*LOG(TE)+(3.37335129D+05)/TE-(2.0822667D+10)/TE**2+(1.0D0)/TE**3)
  rrt(2) = EXP(-5.97861681D+01+(2.06053502D+00)*LOG(TE)+(2.85003919D+05)/TE-(2.08280347D+10)/TE**2+(1.0D0)/TE**3)
  rrt(3) = EXP(-7.17181043D+01+(3.00025937D+00)*LOG(TE)+(5.06454631D+05)/TE-(3.38856621D+10)/TE**2+(1.0D0)/TE**3)
