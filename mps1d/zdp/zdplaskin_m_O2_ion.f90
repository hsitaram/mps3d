...
! config
!
  integer, parameter :: species_max = 6, species_electrons = 1, species_length = 4, reactions_max = 7, reactions_length = 14
...
!
  ...
  integer, parameter, private               :: bolsig_species_max = 1, bolsig_species_length = 2 
...
!
  ...
  /"O2+E=>O2^++E+E","O2+E=>O+O+E   ","O2+E=>O+O^-   ","O+E=>O^++E+E  ","O^-+O2^+=>O+O2","O^-+O^+=>O+O  ","O^-+E=>O+E+E  "/
...
subroutine ZDPlasKin_jex(neq,t,y,ml,mu,pd,nrpd)
  ...
  pd(3,1) = pd(3,1) + rrt(1) * density(6) 
  pd(3,6) = pd(3,6) + rrt(1) * density(1) 
  pd(6,1) = pd(6,1) - rrt(1) * density(6) 
  pd(6,6) = pd(6,6) - rrt(1) * density(1) 
  pd(5,1) = pd(5,1) + rrt(2) * density(6) * 2.0d0
  pd(5,6) = pd(5,6) + rrt(2) * density(1) * 2.0d0
  pd(6,1) = pd(6,1) - rrt(2) * density(6) 
  pd(6,6) = pd(6,6) - rrt(2) * density(1) 
  pd(1,1) = pd(1,1) - rrt(3) * density(6) 
  pd(1,6) = pd(1,6) - rrt(3) * density(1) 
  pd(4,1) = pd(4,1) + rrt(3) * density(6) 
  pd(4,6) = pd(4,6) + rrt(3) * density(1) 
  pd(5,1) = pd(5,1) + rrt(3) * density(6) 
  pd(5,6) = pd(5,6) + rrt(3) * density(1) 
  pd(6,1) = pd(6,1) - rrt(3) * density(6) 
  pd(6,6) = pd(6,6) - rrt(3) * density(1) 
  pd(1,1) = pd(1,1) + rrt(4) * density(5) 
  pd(1,5) = pd(1,5) + rrt(4) * density(1) 
  pd(2,1) = pd(2,1) + rrt(4) * density(5) 
  pd(2,5) = pd(2,5) + rrt(4) * density(1) 
  pd(5,1) = pd(5,1) - rrt(4) * density(5) 
  pd(5,5) = pd(5,5) - rrt(4) * density(1) 
  pd(3,3) = pd(3,3) - rrt(5) * density(4) 
  pd(3,4) = pd(3,4) - rrt(5) * density(3) 
  pd(4,3) = pd(4,3) - rrt(5) * density(4) 
  pd(4,4) = pd(4,4) - rrt(5) * density(3) 
  pd(5,3) = pd(5,3) + rrt(5) * density(4) 
  pd(5,4) = pd(5,4) + rrt(5) * density(3) 
  pd(6,3) = pd(6,3) + rrt(5) * density(4) 
  pd(6,4) = pd(6,4) + rrt(5) * density(3) 
  pd(2,2) = pd(2,2) - rrt(6) * density(4) 
  pd(2,4) = pd(2,4) - rrt(6) * density(2) 
  pd(4,2) = pd(4,2) - rrt(6) * density(4) 
  pd(4,4) = pd(4,4) - rrt(6) * density(2) 
  pd(5,2) = pd(5,2) + rrt(6) * density(4) * 2.0d0
  pd(5,4) = pd(5,4) + rrt(6) * density(2) * 2.0d0
  pd(1,1) = pd(1,1) + rrt(7) * density(4) 
  pd(1,4) = pd(1,4) + rrt(7) * density(1) 
  pd(4,1) = pd(4,1) - rrt(7) * density(4) 
  pd(4,4) = pd(4,4) - rrt(7) * density(1) 
  pd(5,1) = pd(5,1) + rrt(7) * density(4) 
  pd(5,4) = pd(5,4) + rrt(7) * density(1) 
...
subroutine ZDPlasKin_reac_rates(Time)
  ...
  call ZDPlasKin_bolsig_rates()
  Te  = ZDPlasKin_cfg(4)
  rrt(1) = (9.0D-10)*(TE**0.5)*EXP(-146216.7/TE)
  rrt(2) = (4.23D-9)*EXP(-64521.02/TE)
  rrt(3) = (8.8D-11)*EXP(-51059.8/TE)
  rrt(4) = (9.D-09)*(TE**0.7)*EXP(-157821.2/TE)
  rrt(5) = 1.99D-07
  rrt(6) = 2.66D-07
