...
subroutine read_elec_and_gas_energies()
	...
	energyinpfptr = 230
	open(unit=energyinpfptr,file="elec_gas_energy_H2_O2.inp")
	do i = 1,nreac
...
function zdp_getreactionrates(Te, Tgas) result(rrt)
        ...
        rrt(01) = (6.5023D-09)*(TE**0.48931)*EXP(-149624.36/TE) / (eVtoK**0.48931)
        rrt(02) = (2.9962D-08)*(TE**0.44456)*EXP(-437818.75/TE) / (eVtoK**0.44456)
        rrt(03) = (1.0702D-07)*(TE**0.04876)*EXP(-112450.85/TE) / (eVtoK**0.04876)
        rrt(04) = (2.1202D-09)*(TE**0.31394)*EXP(-270371.5/TE) / (eVtoK**0.31394)
        rrt(05) = 9.0D-10
        rrt(06) = 1.19D-22
        rrt(07) = (3.1228D-08)*(TE**0.17156)*EXP(-232987.49/TE) / (eVtoK**0.17156)
        rrt(08) = (1.7527D-7)*(TE**(-1.23668))*EXP(-146128.85/TE) / (eVtoK**(-1.23668))
        rrt(09) = (9.0D-10)*(TE**0.5)*EXP(-146216.7/TE) / (eVtoK**0.5)
        rrt(10) = (4.23D-9)*EXP(-64521.02/TE) / (eVtoK**0.0)
        rrt(11) = (8.8D-11)*EXP(-51059.8/TE)
        rrt(12) = (9.D-09)*(TE**0.7)*EXP(-157821.2/TE) / (eVtoK**0.7)
        rrt(13) = 1.99D-07
        rrt(14) = 2.66D-07
