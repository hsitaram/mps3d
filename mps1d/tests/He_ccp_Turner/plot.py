...
def readfile(filename,enum,nbgspecies,nions,nneutrals,nspecies):
    ...
    gasden     = arr[:,offset:offset+nbgspecies]
    offset += nbgspecies
    elecden  = arr[:,offset]
    offset+=1
    ionden    = arr[:,offset:offset+nions]
    offset+= nions
    nden    = arr[:,offset:offset+nneutrals]
    offset+= nneutrals
    elecenrg = arr[:,offset]
    offset+=1
    electemp  = arr[:,offset]
    ...
    return(x,elecden,ionden,gasden,nden,potential,efield,electemp,jheating,elasticol,inelasticol,electroncurr,ioncurr)
#===================================================================
...
(x,eden,iden,gasden,nden,pot,efield,etemp,ejheat,elcol,inelcol,electroncurr,ioncurr)=readfile(fname,enum,nbgspecies,nions,nneut,len(specnames))
x=x/gapd
