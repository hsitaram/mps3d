...
subroutine setspecparams()
	      ...
	      molmass(1)  =  2.d0*M_PROT
	      molmass(2)  = 32.d0*M_PROT
	      molmass(3)  =       M_ELEC
	      molmass(4)  =  1.d0*M_PROT
	      molmass(5)  =  2.d0*M_PROT
	      molmass(6)  = 16.d0*M_PROT
	      molmass(7)  = 32.d0*M_PROT
	      molmass(8)  = 16.d0*M_PROT
	      molmass(9)  =  1.d0*M_PROT
	      molmass(10) = 16.d0*M_PROT
...
function getspecdcoeff(specnum,specarray,elecfield,Te,Tg,Pg)  result(dcoeff)
	...
	if(specnum .eq. E) then
		...
		dcoeff = k_BMAN*Te*mob/(spec_charge(specnum)*ECHARGE)

	else if(specnum .eq. Hp) then
		
		mob = getspecmobility(specnum,specarray,elecfield,Te,Tg,Pg) 
		dcoeff= k_BMAN*Tg*mob/(spec_charge(specnum)*ECHARGE)
